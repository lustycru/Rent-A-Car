...
class HomeController extends Controller
{
    ...
    public function send(Request $request) {
        ...
        if ($request) {
            if ($data['veh-type']) {
                $allVehiclesWithInnerJoin = DB::select("SELECT v.id AS vehId, v.brand AS vehBrand, v.model AS vehModel, v.year AS vehYear, v.price_per_day AS vehPriceDay, v.doors AS vehDoors, v.fuel_type AS vehFuel, v.air_conditioning AS vehAir, v.seats AS vehSeats, v.transmission AS vehTransmission, v.vehicule_type_id AS vehTypeId, vt.id AS vtId, vt.name AS vtName, vp.image_url AS vehPhotos FROM vehicules v INNER JOIN vehicules_types vt ON v.vehicule_type_id = vt.id INNER JOIN vehicules_photos vp ON v.id = vp.vehicule_id WHERE vt.name = '" . strtolower($data['veh-type']) . "' AND display_order = 0;");
            }

            if ($data['veh-type'] && $data['energy-type']) {
                $allVehiclesWithInnerJoin = DB::select("SELECT v.id AS vehId, v.brand AS vehBrand, v.model AS vehModel, v.year AS vehYear, v.price_per_day AS vehPriceDay, v.doors AS vehDoors, v.fuel_type AS vehFuel, v.air_conditioning AS vehAir, v.seats AS vehSeats, v.transmission AS vehTransmission, v.vehicule_type_id AS vehTypeId, vt.id AS vtId, vt.name AS vtName, vp.image_url AS vehPhotos FROM vehicules v INNER JOIN vehicules_types vt ON v.vehicule_type_id = vt.id INNER JOIN vehicules_photos vp ON v.id = vp.vehicule_id WHERE vt.name = '" . strtolower($data['veh-type']) . "' AND v.fuel_type = '" . strtolower($data['energy-type']) . "' AND display_order = 0;");
            }

            if ($data['veh-type'] && $data['gear-type']) {
                $allVehiclesWithInnerJoin = DB::select("SELECT v.id AS vehId, v.brand AS vehBrand, v.model AS vehModel, v.year AS vehYear, v.price_per_day AS vehPriceDay, v.doors AS vehDoors, v.fuel_type AS vehFuel, v.air_conditioning AS vehAir, v.seats AS vehSeats, v.transmission AS vehTransmission, v.vehicule_type_id AS vehTypeId, vt.id AS vtId, vt.name AS vtName, vp.image_url AS vehPhotos FROM vehicules v INNER JOIN vehicules_types vt ON v.vehicule_type_id = vt.id INNER JOIN vehicules_photos vp ON v.id = vp.vehicule_id WHERE vt.name = '" . strtolower($data['veh-type']) . "' AND v.transmission = '" . strtolower($data['gear-type']) . "' AND display_order = 0;");
            }

            if ($data['energy-type'] && $data['gear-type']) {
                $allVehiclesWithInnerJoin = DB::select("SELECT v.id AS vehId, v.brand AS vehBrand, v.model AS vehModel, v.year AS vehYear, v.price_per_day AS vehPriceDay, v.doors AS vehDoors, v.fuel_type AS vehFuel, v.air_conditioning AS vehAir, v.seats AS vehSeats, v.transmission AS vehTransmission, v.vehicule_type_id AS vehTypeId, vt.id AS vtId, vt.name AS vtName, vp.image_url AS vehPhotos FROM vehicules v INNER JOIN vehicules_types vt ON v.vehicule_type_id = vt.id INNER JOIN vehicules_photos vp ON v.id = vp.vehicule_id WHERE v.fuel_type = '" . strtolower($data['energy-type']) . "' AND v.transmission = '" . strtolower($data['gear-type']) . "' AND display_order = 0;");
            }

            if ($data['veh-type'] && $data['energy-type'] && $data['gear-type']) {
                $allVehiclesWithInnerJoin = DB::select("SELECT v.id AS vehId, v.brand AS vehBrand, v.model AS vehModel, v.year AS vehYear, v.price_per_day AS vehPriceDay, v.doors AS vehDoors, v.fuel_type AS vehFuel, v.air_conditioning AS vehAir, v.seats AS vehSeats, v.transmission AS vehTransmission, v.vehicule_type_id AS vehTypeId, vt.id AS vtId, vt.name AS vtName, vp.image_url AS vehPhotos FROM vehicules v INNER JOIN vehicules_types vt ON v.vehicule_type_id = vt.id INNER JOIN vehicules_photos vp ON v.id = vp.vehicule_id WHERE v.fuel_type = '" . strtolower($data['energy-type']) . "' AND v.transmission = '" . strtolower($data['gear-type']) . "' AND vt.name = '" . strtolower($data['veh-type']) . "' AND display_order = 0;");
            }
        }
